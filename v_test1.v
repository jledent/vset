...
Definition bisimulation : V@{Top42} -> V@{Top42} -> hProp@{Top42}.
Proof.
...
Lemma BisimEqualsId : forall u v : V@{Top42}, (u = v) = (u ~~ v).
Proof.
  intros u v.
  Check (u ~~ v). (* this wouldn't typecheck without fixing universes *)

  (* apply path_iff_hprop_uncurried. -> What I would like to do, doesn't work *)

  assert (H : IsHProp (u ~~ v)) by apply isp.
   (* refine (@path_iff_hprop_uncurried ua (u = v) _ (u ~~ v) H _). -> doesn't work *)

  refine (@path_iff_hprop_uncurried ua (u = v) _ (u ~~ v) _ _).
   (* apply isp.  doesn't work *)
